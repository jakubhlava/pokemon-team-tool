...
function isSkippable(pokemonName: string): boolean {
    const exceptions = [
        'maushold',
        'squawkabilly',
        'palafin',
    ...
    if (pokemonName.endsWith('-mega') || pokemonName.endsWith('-mega-x') || pokemonName.endsWith('-mega-y') || pokemonName.endsWith('-gmax')) {
        matched = true;
    }
    ...
    while (true) {
        try {
            ...
            allPokemon.forEach(async (pokemon) => {
                ...
                const pokeInfo = await PokemonApi.pokemon.getPokemonByName(pokemon.name).then((response) => {
                    return {
                ...
                const speciesInfo = await PokemonApi.pokemon.getPokemonSpeciesByName(pokeInfo.species).then((response) => {
                    return {
                ...
                speciesInfo.varieties = speciesInfo.varieties.filter((v) => !v.pokemon.name.endsWith('-gmax'))
                    .filter((v) => !v.pokemon.name.endsWith('-mega'))
                    .filter((v) => !v.pokemon.name.endsWith('-mega-x'))
                    .filter((v) => !v.pokemon.name.endsWith('-mega-y'))

                if (speciesInfo.varieties.length === 1 || speciesInfo.varieties[0].pokemon.name === 'pikachu' || pokeInfo.forms.length > 1) {
                ...
                speciesInfo.varieties.forEach(async (varieties) => {
                    const formInfo = await PokemonApi.pokemon.getPokemonFormByName(varieties.pokemon.name).then((response) => {
                        return {
                            formName: response.names.find((name) => name.language.name === 'en')?.name,
                            types: response.types
                        };
                    });
